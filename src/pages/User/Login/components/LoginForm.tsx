...
export const LoginFormUI = (props: LoginFormProps) => {
    ...
    const fetchUserInfo = async (loginResult: AppModels.LoginResult) => {
        let userInfo: AppModels.CurrentUser | undefined = loginResult.currentUser;
        if (!userInfo) {
            userInfo = await initialState?.fetchUserInfo?.();
        }
        if (userInfo) {
            flushSync(() => {
                setInitialState((s) => ({
    ...
    const intl = useIntl();
    const { status, errorTips } = userLoginState;
    ...
    return (
        ...
        >
            ...
            {status === 'error' && loginType === 'account' && (
                <LoginMessage
                    content={errorTips ?? intl.formatMessage({
